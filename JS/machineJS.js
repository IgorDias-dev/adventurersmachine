function calcular() {
    char.innerHTML = `<img style="animation-interation-count: 1;" src="./Assets/characterdamage.gif" alt="">`; // altera o gif do personagem/ botão
    ...
    if (document.getElementById('convertDecimal').checked) {
        ...
        outputHTML += `<p>${decimalValue}<sub>Decimal</sub></p>`; // adiciona o resultado ao html
    }
    if (document.getElementById('convertHexadecimal').checked) {
        output.style.display = 'block'; // exibe a saída
        outputHTML += `<p>${decimalValue.toString(16).toUpperCase()}<sub>Hexadecimal</sub></p>`; // converte para hexadecimal e adiciona ao html
    }
    if (document.getElementById('convertOctal').checked) {
        output.style.display = 'block'; // exibe a saída
        outputHTML += `<p>${decimalValue.toString(8)}<sub>Octal</sub></p>`; // converte para octal e adiciona ao html
    }
    if (document.getElementById('convertBinario').checked) {
        output.style.display = 'block'; // exibe a saída
        outputHTML += `<p>${decimalValue.toString(2)}<sub>Binário</sub></p>`; // converte
    }
